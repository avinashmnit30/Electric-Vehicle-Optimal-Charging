...
conv_data=numpy.zeros(shape=(11,))
...
#        print SMnum
    ...
    SMnum=SMLP
    while SMnum<SMUP:
        SMval[SMnum][0],fval,penalty1,penalty2,penalty3=func(p_sol,SM[SMnum],veh_no,d.T,mode=mode)
...
counterk1=0
for k1 in [3.3,4.4,5.5,6.6,7.7,8.8]:
    ...
    for veh_no in range(N_veh):
        ...
        for run_no in range(runs):
                ...
                if GLc>=GLL:
                    ...
                    while grp<=MSw:
                        ...
                        while SMnum<Sw[MSw-1][2*grp]:
                            ...
                            SMnum+=1
                        grp+=1
                GLval,fval,penalty1,penalty2,penalty3=func(p_sol,GL,veh_no,d.T,mode=mode)
                print 'it=',it,' MSw=',MSw,' GLval=',GLval,fval,penalty1,penalty2,penalty3
                if it%50==0:
                    conv_data[it/50]+=fval
                if it==499:
                    conv_data[10]+=fval
            ...
            time2=time.time()
            run_no+=1
            d_temp=func(p_sol,GL,veh_no,d.T,mode=mode,get_d=1)
            d[veh_no]=d_temp
            p_sol[veh_no]=GL
...
print('Total_revenue=',numpy.sum(value))
        
numpy.savetxt('ASMO'+str(N_veh)+"dynamicev"+'mode'+str(mode)+".csv",value,delimiter=",")            
numpy.savetxt('ASMO'+str(N_veh)+"conv"+'mode'+str(mode)+".csv",conv_data,delimiter=",")            
